...
function applyWatermark(src) {
  return new Promise(resolve => {
    const img = new Image();
    // try with crossOrigin first; if it fails, fall back to plain src
    ...
    img.onerror = () => {
      // crossOrigin request failed — retry without it
      const img2 = new Image();
      img2.onload = () => resolve(src); // show without watermark
      img2.onerror = () => resolve(src);
      img2.src = src;
    };
...
async function loadGrid(jsonPath, containerId, limit) {
  ...
  try {
    ...
    if (data[0] && data[0].photos) {
      photos = data.flatMap(group => group.photos);
    } else {
      photos = data;
    }
